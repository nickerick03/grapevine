-- Add structured report reasons/details for note flags and expose them to admin flag triage.
...
set reason = coalesce(reason, 'other')
...
alter table public.place_rating_note_flags
  drop constraint if exists place_rating_note_flags_reason_check;

alter table public.place_rating_note_flags
  add constraint place_rating_note_flags_reason_check
  check (reason in ('incorrect', 'false', 'inappropriate', 'other'));

alter table public.place_rating_note_flags
  drop constraint if exists place_rating_note_flags_details_len_check;

alter table public.place_rating_note_flags
  add constraint place_rating_note_flags_details_len_check
  check (details is null or char_length(details) <= 280);
